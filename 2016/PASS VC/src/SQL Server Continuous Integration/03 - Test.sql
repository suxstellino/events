...
BEGIN
	...
	-- insert "stub" records
	INSERT INTO dbo.Customers (CustomerId, FirstName, LastName, BirthDate, Gender)
	VALUES
			(1, 'John', 'Fittizio', '19700505', 'M');
...
CREATE PROCEDURE UserTests.[test Total number of records should be the same of the number of details]
AS
BEGIN
	...
	DECLARE @Counter int;
	SELECT @Counter = COUNT(1) FROM @Results;
	
	-- Assert
	EXEC tSQLt.AssertEquals
	    @Expected = 3
	  , @Actual = @Counter
	  , @Message = N'Wrong number of records!';
	

END;
GO

CREATE PROCEDURE UserTests.[test Customer update should accept M or F on Gender column]
AS
BEGIN
	...
	EXEC tSQLt.ApplyConstraint
	    @TableName = N'Customers'
	  , @ConstraintName = N'CK_Customers_Gender';	
